...
function twb_settings_register(){
	register_setting('twb_wc_reviews_settings', 'twb_wc_reviews_option', '' );
		...
		add_settings_field('twb_random_order', 'Randomize Comments Display?', 'twb_random_order_fn', __FILE__, 'twb_section');
...
function twb_random_order_fn() {
	...
	echo'<input type="checkbox" id="twb_random_order" name="twb_wc_reviews_option[twb_random_order]" value="1" ' . checked(1, $options['twb_random_order'], false) . '/> Select this checkbox if you want to display comments in random order.';
...
?>
  ...
  <div class="postbox" style="float:right; width: 25%; padding:10px;">
    ...
    <p><a href="https://theweb-designs.com/product/donate-to-twb-woocommerce-reviews-plugin/">Click here to go to donation page.</a></p>
  </div>
  <div class="postbox" style="float:right; width: 25%; padding:10px;">
  <h2 style="line-height:1.5;">WordPress Site Maintenance, malware removal, backups, version updates, speed optimization, referrer spam blocking ... - only $50 per month per site.</h2>
	<h3><a href="https://theweb-designs.com/wmo/" target="_blank">Click for more details</a></h3>
	</div>
</div>
<?php
}